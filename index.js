...
function generateTableMd(reposTemplateStr, reposArray) {
	...
	reposArray.forEach((repo, i) => {
		...
		generatedRepoTemplate += `| ${i + 1} | ${repo.stargazers_count} | [${repo.full_name}](${repo.html_url}) | ${repo.description} | [<div style="width:25px;">![${repo.owner_login}](${repo.owner_avatar_url})</div>](${repo.owner_html_url}) | ${moment(repo.created_at).format('YYYY-MM-DD').toString()} |\n`;
	});
